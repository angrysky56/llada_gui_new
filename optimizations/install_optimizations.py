...
def main():
    ...
    llada_dir = script_dir.parent
    print(f"LLaDA GUI directory: {llada_dir}")
    
    # Check if we're in the right directory structure
    if not (llada_dir / "llada_gui.py").exists():
        print("Error: Cannot find llada_gui.py in parent directory.")
        print("Please run this script from the optimizations directory within the LLaDA GUI repository.")
        return 1
    ...
    if not args.no_backup:
        ...
        for file in ["config.py", "llada_worker.py"]:
            if (llada_dir / file).exists():
                backup_path = llada_dir / f"{file}.backup"
                shutil.copy2(llada_dir / file, backup_path)
                print(f"  - Created backup: {backup_path}")
    
    # Update config.py
    print("Updating config.py...")
    config_path = llada_dir / "config.py"
...
"""
    ...
    worker_path = llada_dir / "llada_worker.py"
    ...
    try:
        ...
        if "OPTIMIZED_GPU_MEMORY" in worker_content:
            ...
            with open(llada_dir / "optimized_worker.py", "w") as f:
                # Start with imports
            ...
            with open(llada_dir / "optimized_worker.py", "r") as f:
                content = f.read()
            ...
            if "model_load_params[\"torch_dtype\"] = torch.bfloat16 if device == 'cuda' else torch.float32" in content:
                ...
                shutil.move(llada_dir / "optimized_worker.py", worker_path)
                ...
                # Clean up
                if (llada_dir / "optimized_worker.py").exists():
                    os.remove(llada_dir / "optimized_worker.py")
    ...
    # Update desktop file
    print("Creating optimized desktop shortcut...")
    try:
        desktop_path = llada_dir / "LLaDA_Optimized.desktop"
        desktop_content = f"""[Desktop Entry]
Version=1.0
Type=Application
Name=LLaDA GUI (Optimized)
Comment=Optimized LLaDA GUI with improved memory usage
Exec={sys.executable} {llada_dir}/run.py
